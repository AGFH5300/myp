...
export default function PapersPage() {
  ...
  useEffect(() => {
    supabase
      .from('papers')
      .select('id,title,subject_id,subject:subjects(name),session:exam_sessions(exam_year,exam_month)')
      .eq('is_published', true)
      .then(({ data }) => setPapers((data as never) ?? []));
  }, [supabase]);
  ...
  return (
    <div>
      ...
      <Card>
        ...
        {filtered.length === 0 ? (
          <p className="text-sm text-slate-600">No published papers match your filters.</p>
        ) : (
          <div className="space-y-2">
            {filtered.map((paper) => (
              <Link key={paper.id} href={`/papers/${paper.id}`} className="block rounded border p-3 hover:bg-slate-50">
