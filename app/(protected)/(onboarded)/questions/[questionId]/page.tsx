...
export default function QuestionPage() {
  ...
  if (!question) return <div className="p-2 text-sm text-slate-600">Question not found or not published.</div>;
      ...
      <Card>
        ...
        <form onSubmit={saveAnswer} className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" className="bg-slate-700" onClick={async () => attemptId && supabase.from('attempts').update({ status: 'submitted', submitted_at: new Date().toISOString() }).eq('id', attemptId)}>Submit</Button>
            <Button type="button" className="bg-emerald-700" onClick={async () => { setShowSolution(true); if (attemptId) await supabase.from('attempts').update({ revealed_solution_at: new Date().toISOString() }).eq('id', attemptId); }}>Reveal solution</Button>
            <Button type="button" className="bg-amber-700" onClick={async () => {
              if (!session?.user || !questionId) return;
              if (bookmarked) {
            ...
            }}>{bookmarked ? 'Remove bookmark' : 'Bookmark question'}</Button>
          </div>
        </form>
        {showSolution && question.markscheme_text && <div className="mt-4 rounded border border-emerald-200 bg-emerald-50 p-3"><h3 className="font-medium">Markscheme</h3><p className="whitespace-pre-wrap text-sm">{question.markscheme_text}</p></div>}
      </Card>
    </div>
  );
}
